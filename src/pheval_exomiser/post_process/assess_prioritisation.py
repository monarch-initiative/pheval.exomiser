...
from pheval.utils.phenopacket_utils import (
    PhenopacketUtil, VariantData, phenopacket_reader,
...
class SimplifiedExomiserGeneResult:
    ...
    def add_gene_record(self) -> dict:
        ...
        return {"gene_symbol": self.exomiser_result["geneSymbol"],
...
class SimplifiedExomiserVariantResult:
    ...
    def create_simplified_variant_result(self) -> [dict]:
        ...
        for cv in self.exomiser_result["contributingVariants"]:
            self.simplified_exomiser_variant_result.append({"variant":
                VariantData(
                    cv["contigName"],
                    cv["start"],
                    cv["ref"],
    ...
    def simplify_gene_result(self) -> [dict]:
        ...
        for result in self.exomiser_json_result:
            if self.ranking_method in result:
                simplified_exomiser_result = SimplifiedExomiserGeneResult(result, result["geneSymbol"],
                                                                          simplified_exomiser_result,
                                                                          self.ranking_method).create_simplified_gene_result()
            ...
            for gene_hit in result["geneScores"]:
                if self.ranking_method in gene_hit:
                    if "contributingVariants" in gene_hit:
                        simplified_exomiser_result = SimplifiedExomiserVariantResult(gene_hit,
                                                                                     simplified_exomiser_result,
                                                                                     self.ranking_method,
                                                                                     round(result[
                                                                                               self.ranking_method],
                                                                                           4)).create_simplified_variant_result()
        return simplified_exomiser_result
...
class AssessExomiserGenePrioritisation:
    ...
    def __init__(self, phenopacket_path: Path, results_dir: Path, standardised_exomiser_gene_result: [dict],
                 threshold: float, ranking_method: str, proband_causative_genes: [ProbandCausativeGene]):
    ...
    def record_gene_prioritisation_match(
            self, gene: ProbandCausativeGene, result_data: dict, rank_stats: RankStats
    ) -> GenePrioritisationResultData:
    ...
    def assess_gene_with_pvalue_threshold(
            self, result_data: dict, gene: ProbandCausativeGene, rank_stats: RankStats
    ) -> GenePrioritisationResultData:
    ...
    def assess_gene_prioritisation(self, rank_stats: RankStats, rank_records: defaultdict):
        ...
        for gene in self.proband_causative_genes:
            ...
            for exomiser_result in self.standardised_exomiser_gene_result:
                if gene.gene_identifier == exomiser_result["gene_identifier"] and float(self.threshold) != 0.0:
                    gene_match = (
                        self.assess_gene_with_threshold(exomiser_result, gene, rank_stats)
                        if self.ranking_method != "pValue"
                        else self.assess_gene_with_pvalue_threshold(exomiser_result, gene, rank_stats)
                    )
                    break
                if gene.gene_identifier == exomiser_result["gene_identifier"] and float(self.threshold) == 0.0:
                    ...
                    break
            PrioritisationRankRecorder(
                rank_stats.total, self.results_dir, gene_match, rank_records
            ).record_rank()
...
class AssessExomiserVariantPrioritisation:
    ...
    def __init__(self, phenopacket_path: Path, results_dir: Path, standardised_exomiser_variant_result: [dict],
                 threshold: float, ranking_method: str, proband_causative_variants: [VariantData]):
        self.phenopacket_path = phenopacket_path
        self.results_dir = results_dir
    ...
    def record_variant_prioritisation_match(
        ...
        rank = result_data["rank"]
        rank_stats.add_rank(rank)
        variant_match = VariantPrioritisationResultData(self.phenopacket_path, result_data["variant"], rank)
        return variant_match

    def assess_variant_with_pvalue_threshold(
            self, result_data: dict, rank_stats: RankStats
    ) -> VariantPrioritisationResultData:
        """Records the variant prioritisation rank if it meets the pvalue threshold."""
        if float(self.threshold) > float(result_data[self.ranking_method]):
            return self.record_variant_prioritisation_match(result_data, rank_stats)
        ...
        for variant in self.proband_causative_variants:
            ...
            for result in self.standardised_exomiser_variant_result:
                ...
                if (
                    ...
                    break
            PrioritisationRankRecorder(
                rank_stats.total, self.results_dir, variant_match, rank_records
            ).record_rank()
...
def assess_phenopacket_gene_prioritisation(exomiser_result: Path, ranking_method: str,
                                           results_dir_and_input: CorrespondingExomiserInput,
                                           threshold: float, gene_rank_stats: RankStats,
                                           gene_rank_comparison: defaultdict):
    phenopacket_path = obtain_closest_file_name(exomiser_result, all_files(results_dir_and_input.phenopacket_dir))
    ...
    AssessExomiserGenePrioritisation(phenopacket_path, results_dir_and_input.results_dir,
                                     standardised_exomiser_gene_result,
                                     threshold, ranking_method, proband_causative_genes).assess_gene_prioritisation(
        gene_rank_stats, gene_rank_comparison)


def assess_phenopacket_variant_prioritisation(exomiser_result: Path, ranking_method: str,
                                              results_dir_and_input: CorrespondingExomiserInput,
                                              threshold: float, variant_rank_stats: RankStats,
    ...
    AssessExomiserVariantPrioritisation(phenopacket_path, results_dir_and_input.results_dir,
                                        standardised_exomiser_variant_result, threshold, ranking_method,
                                        proband_causative_variants).assess_variant_prioritisation(
...
def assess_prioritisation_for_results_directory(
    ...
    for exomiser_result in exomiser_json_results:
        if gene_analysis and not variant_analysis:
            assess_phenopacket_gene_prioritisation(exomiser_result, ranking_method, results_directory_and_input,
                                                   threshold, gene_rank_stats, gene_rank_comparison)
        elif variant_analysis and not gene_analysis:
            assess_phenopacket_variant_prioritisation(exomiser_result, ranking_method, results_directory_and_input,
                                                      threshold, variant_rank_stats, variant_rank_comparison)
        elif variant_analysis and gene_analysis:
            assess_phenopacket_gene_prioritisation(exomiser_result, ranking_method, results_directory_and_input,
                                                   threshold, gene_rank_stats, gene_rank_comparison)
            assess_phenopacket_variant_prioritisation(exomiser_result, ranking_method, results_directory_and_input,
                                                      threshold, variant_rank_stats, variant_rank_comparison)
    gene_stats_writer.write_row(results_directory_and_input.results_dir, gene_rank_stats) if gene_analysis else None
    variants_stats_writer.write_row(results_directory_and_input.results_dir,
                                    variant_rank_stats) if variant_analysis else None
...
def benchmark_directory(
    ...
    gene_stats_writer = RankStatsWriter(Path(output_prefix + "-gene_summary.tsv")) if gene_analysis else None
    variants_stats_writer = RankStatsWriter(Path(output_prefix + "-variant_summary.tsv")) if variant_analysis else None
    ...
    RankComparisonGenerator(gene_rank_comparison).generate_gene_output(
        f"{results_dir_and_input.results_dir.name}") if gene_analysis else None
    RankComparisonGenerator(variant_rank_comparison).generate_variant_output(
        f"{results_dir_and_input.results_dir.name}") if variant_analysis else None
    gene_stats_writer.close() if gene_analysis else None
    variants_stats_writer.close() if variant_analysis else None


def benchmark_directories_for_pairwise_comparison(results_directories: [CorrespondingExomiserInput],
                                                  ranking_method: str,
                                                  output_prefix: str,
                                                  threshold: float,
                                                  gene_analysis: bool,
    ...
    RankComparisonGenerator(gene_rank_comparison).generate_gene_comparison_output(
        f"{results_directories[0].results_dir.name}__v__{results_directories[1].results_dir.name}") if gene_analysis else None
...
def benchmark_several_directories(results_directories: [CorrespondingExomiserInput], ranking_method: str,
                                  output_prefix: str,
                                  threshold: float,
                                  gene_analysis: bool,
                                  variant_analysis: bool):
    ...
    for results_dir_and_input in results_directories:
        ...
        gene_ranks_for_directories.append(TrackGeneComparisons(results_dir_and_input.results_dir, gene_ranks))
        variant_ranks_for_directories.append(TrackVariantComparisons(results_dir_and_input.results_dir, variant_ranks))
    generate_gene_rank_comparisons(
        list(itertools.combinations(gene_ranks_for_directories, 2))) if gene_analysis else None
    generate_variant_rank_comparisons(
        list(itertools.combinations(variant_ranks_for_directories, 2))) if variant_analysis else None
...
@click.option('--gene-analysis/--no-gene-analysis', default=True, required=False, type=bool, show_default=True,
              help="Analyse gene prioritisation")
@click.option('--variant-analysis/--no-variant-analysis', default=True, required=False, type=bool, show_default=True,
              help="Analyse variant prioritisation")
def benchmark(directory: Path, phenopacket_dir: Path, ranking_method, output_prefix, threshold, gene_analysis,
              variant_analysis):
    benchmark_directory(CorrespondingExomiserInput(results_dir=directory, phenopacket_dir=phenopacket_dir),
                        ranking_method, output_prefix, threshold, gene_analysis, variant_analysis)
